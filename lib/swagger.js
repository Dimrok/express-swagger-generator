...
const BASE_TYPES = ['object', 'string', 'integer', 'number', 'boolean'];
...
function parseDesc(properties) {
  const split = (properties.description || '').split(/-\s*eg:\s*/);
  const description = split[0].trim();
  const example = split[1];
  if (example) {
    try {
      return {
        example: JSON.parse(example.trim()),
        description
      };
    } catch (error) {
      return {
    ...
    if (properties.type && properties.type.name) {
      return {
        description,
        example: {
          $ref: `#/definitions/${properties.type.name}.example`
        }
      };
    }
  }
...
function parseTag(tags) {
  for (let i in tags) {
    if (tags[i]['title'] == 'group') {
      return tags[i]['description'].split('-').map(t => t.trim());
    }
  }
  return ['default', ''];
}
...
function parseTypedef(tags) {
  ...
  if (tags[0].title === 'typedef') {
    if (tags[0].type) {
      ...
      if (type.type === 'NameExpression') {
        ...
        details.type = type.expression.name;
      }
    }
  }
  for (let i = 1; i < tags.length; i++) {
    if (tags[i].title == 'description') {
      const { description, example } = parseDesc(tags[i]);
      details.description = description;
      details.example = example;
    } else if (tags[i].title == 'tags') {
      details.tags = tags[i].description.split(',').map(t => t.trim());
    } else if (tags[i].title == 'property') {
        ...
        if (example) {
          switch (type) {
            ...
            case 'number':
              ...
              break;
            case 'object':
              details.properties[propName].example = JSON.parse(example);
              break;
            case 'enum':
              break;
            case 'string':
  ...
  if (details.required.length === 0) delete details.required;
  if (Object.keys(details.properties).length === 0) delete details.properties;
    ...
    for (let i = 0; i < apiFiles.length; i = i + 1) {
      ...
      for (let j in comments) {
        try {
          ...
          console.log(
            `Incorrect comment format ${e}. Method was not documented.\nFile: ${apiFiles[i]}\nComment:`,
            comments[j]
          );
        }
      }
