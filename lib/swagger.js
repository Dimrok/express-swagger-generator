...
const BASE_TYPES = ['object', 'string', 'number', 'integer', 'boolean'];
...
function parseExample(properties) {
  if (!properties.example) {
    const parsed = (properties.description || '').split(/-\s*eg:\s*/);
    properties.description = parsed[0].trim();
    const example = parsed[1];
    if (example) {
      try {
        properties.example = JSON.parse(example.trim());
      } catch (error) {
        properties.example = example.trim();
      }
    }
  }
  return properties;
...
function parseDefault(properties) {
  const parsed = (properties.description || '').split(/-\s*default:\s*/);
  properties.description = parsed[0].trim();
  const def = parsed[1];
  if (def) {
    try {
      properties.default = JSON.parse(def.trim());
    } catch (error) {
      properties.default = ef.trim();
    }
  }
  return properties;
}
    ...
    if (tags[i].title == 'property') {
      ...
      if (schema) {
        ...
        if (example) {
          switch (type) {
            case 'boolean':
              details.properties[propName].example =
        ...
        if (title == 'param') {
          ...
          if (!schema) {
            ...
            if (properties.type == 'enum') {
              ...
              properties = parseExample(properties);
            }
          } else {
